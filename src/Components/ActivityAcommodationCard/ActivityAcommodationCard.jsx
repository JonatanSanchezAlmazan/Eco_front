    ...
    <div className="activityAcommodationCard">
      ...
      {item.map((i, index) => (
        <div className="activityAcommodationCard__content" key={index}>
          ...
          {showConfirm && <ConfirmModal text={`Seguro que quieres eliminar ${i.name}`} setShow={setShowConfirm} remove={() => (title === 'Actividades' ? deleteActivity({ dispatch: activitiesDispatch, id: i._id }) : '')} />}
        </div>
      ))}
